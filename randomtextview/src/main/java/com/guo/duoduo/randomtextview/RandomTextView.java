...
public class RandomTextView extends FrameLayout
    ...
    @TargetApi(21)
    public RandomTextView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public void addKeyWord(String keyword) {
        if (vecKeywords.size() < MAX) {
            if (!vecKeywords.contains(keyword)) vecKeywords.add(keyword);
        }
    }
    ...
    public void show() {
        ...
        if (width > 0 && height > 0 && vecKeywords != null &&
            ...
            for (int i = 0; i < size; i++) {
                ...
                txt.setOnClickListener(new OnClickListener() {
                    @Override public void onClick(View view) {
                        if (onRippleOutViewClickListener != null) {
                            onRippleOutViewClickListener.onRippleViewClicked(
                                    view);
                        }
                    }
                });
                ...
                int strWidth
                        = /* (int) Math.ceil(paint.measureText(keyword)) */txt.getMeasuredWidth();
    ...
    private void attach2Screen(LinkedList<RippleView> listTxt, int xCenter, int yCenter, int yItem) {
...
inner:
            for (int k = i - 1; k >= 0; k--) {
                ...
                if (yDistance * (kXY[IDX_Y] - yCenter) > 0) {
                    if (isXMixed(startX, endX, iXY[IDX_X],
    ...
    private void sortXYList(LinkedList<RippleView> listTxt, int endIdx) {
        for (int i = 0; i < endIdx; i++) {
            for (int k = i + 1; k < endIdx; k++) {
                if (((int[]) listTxt.get(k).getTag())[IDX_DIS_Y] <
                        ((int[]) listTxt.get(i).getTag())[IDX_DIS_Y]) {
